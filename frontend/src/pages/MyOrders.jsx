...
//                         className="text-white hover:text-gray-300 font-bold uppercase tracking-wider text-[11px] border border-white/20 hover:border-white/50 px-4 py-2 rounded-full transition-all"
//                       >
//                         View Details
//                       </Link>
  ...
  return (
    <div className="min-h-screen w-full bg-[#08090C] text-white font-['Plus_Jakarta_Sans'] selection:bg-white selection:text-black">
      ...
      <main className="max-w-[1200px] mx-auto px-6 sm:px-12 py-12">
        ...
        {!loading && !error && allOrders.length > 0 && (
          <div className="mb-8 max-w-md">
            <div className="relative flex items-center">
              ...
              <input
                type="text"
                placeholder="Search by Order ID (e.g., 8A1B2C3D)..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-[#0E1015] border border-white/10 rounded-full pl-11 pr-10 py-3 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-white/40 transition-all"
              />
              {searchQuery && (
                <button
        ...
        {/* No Orders or No Search Results */}
        {!loading && !error && orders.length === 0 && (
          <div className="bg-[#0E1015] border border-white/10 rounded-3xl p-16 text-center max-w-lg mx-auto">
            <ShoppingBag
              size={40}
            ...
            <h3 className="text-xl font-bold text-white">
              {searchQuery ? "No Matching Orders Found" : "No Orders Placed"}
            </h3>

            <p className="text-sm text-gray-400 mt-2 mb-6">
              {searchQuery
                ? `No orders match your search term "${searchQuery}". Please check the Order ID.`
