  ...
  if (loading || !order) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-[#F9F9F9] px-4 text-center">
        <p className="text-sm text-[#5E5E5E]">{error || 'Loading order...'}</p>
      </main>
    );
  }
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur px-4 py-6 sm:px-6 sm:py-7 lg:px-10 lg:py-8 border-b border-[#EEEEEE]">
        <div className="max-w-[1440px] w-full mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-4 sm:gap-6 min-w-0">
            <a onClick={() => navigate('/admin/orders')} className="flex items-center gap-2 text-[11px] font-medium uppercase text-[#5E5E5E] hover:text-[#1A1C1C] transition-colors duration-200 cursor-pointer whitespace-nowrap">
              <ArrowLeft size={20} />
              Orders Archive
            </a>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 min-w-0">
              <h1 className="text-[22px] sm:text-[26px] lg:text-[32px] font-normal tracking-tight text-[#1A1C1C] break-all" style={{ fontFamily: "'Libre Caslon Text', serif" }}>
                Order #{order._id}
              </h1>
          ...
          <div className="flex justify-end w-full md:w-auto">
            <button onClick={() => window.print()} className="w-full md:w-auto px-6 py-3 border border-black bg-transparent text-[11px] uppercase tracking-wide transition-all duration-300 hover:bg-[#F3F3F4]">
              Print Invoice
            </button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 sm:gap-12">
            <div className="lg:col-span-7 flex flex-col gap-10 sm:gap-12 lg:gap-16">
              <section className="transition-all duration-700">
                <p className="text-[11px] uppercase font-medium text-[#5E5E5E] mb-6 sm:mb-8">Product Details</p>
                <div className="flex flex-col gap-6">
                  {(order.items || []).map((item, i) => (
                    <div key={item._id || i} className="bg-white p-5 sm:p-8 flex flex-col md:flex-row gap-6 sm:gap-8">
                      <div className="w-full md:w-48 aspect-square overflow-hidden bg-[#EEEEEE] flex-shrink-0">
                        <img src={item.image || item.product?.mainImage} alt={item.productName || item.product?.modelName} className="w-full h-full object-cover" />
                      </div>
                      <div className="flex-1 flex flex-col justify-between min-w-0">
                        <div>
                          <p className="text-xl sm:text-2xl text-[#1A1C1C] break-words" style={{ fontFamily: "'Libre Caslon Text', serif" }}>
                            {item.productName || item.product?.modelName || 'Unknown product'}
                          </p>
                        ...
                        <div className="flex flex-wrap justify-between items-center gap-2 pt-6 mt-6 sm:pt-8 sm:mt-8 border-t border-[#CFC4C5]">
                          <span className="text-[11px] uppercase text-[#5E5E5E]">Unit Price · Qty {item.quantity}</span>
                          <span className="text-xl sm:text-2xl text-[#1A1C1C]" style={{ fontFamily: "'Libre Caslon Text', serif" }}>
              ...
              <section className="transition-all duration-700">
                <p className="text-[11px] uppercase font-medium text-[#5E5E5E] mb-6 sm:mb-8">Order Summary</p>
                <div className="bg-[#F3F3F4] p-5 sm:p-8 flex flex-col gap-5 sm:gap-6">
                  ...
                  <div className="flex flex-wrap justify-between items-center gap-2">
                    ...
                    <span className="text-3xl sm:text-4xl lg:text-5xl text-[#1A1C1C] break-words" style={{ fontFamily: "'Libre Caslon Text', serif" }}>
            ...
            <div className="lg:col-span-5 flex flex-col gap-10 sm:gap-12 lg:gap-16">
              <section className="lg:mt-16">
                <p className="text-[11px] uppercase font-medium text-[#5E5E5E] mb-6 sm:mb-8">Customer Information</p>
                <div className="bg-white p-5 sm:p-8 flex flex-col gap-6 sm:gap-8">
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <span className="text-base font-bold text-[#1A1C1C] break-words">{customer.name}</span>
                  </div>
                  <div className="border-t border-[#CFC4C5] pt-6 grid grid-cols-1 gap-6">
                    <p className="text-sm text-[#5E5E5E] break-all">{customer.email}</p>
                    <p className="text-sm text-[#5E5E5E] break-words">{customer.phone}</p>
                  </div>
                </div>
              </section>

              <section>
                <p className="text-[11px] uppercase text-[#5E5E5E] mb-6 sm:mb-8">Shipping Address</p>
                <div className="bg-white p-5 sm:p-8">
                  <div className="flex flex-col gap-2">
                    <p className="text-base text-[#1A1C1C] break-words">{order.shippingAddress?.address}</p>
              ...
              <section>
                <p className="text-[11px] uppercase text-[#5E5E5E] mb-6 sm:mb-8">Payment Method</p>
              ...
              {actionError && <p className="text-sm text-[#A32D2D]">{actionError}</p>}

              <div className="pt-8 border-t border-[#CFC4C5]">
                <div className="flex flex-col sm:flex-row gap-4">
                  <button onClick={handleCancelOrder} disabled={updating || !canCancel} className="w-full py-4 border border-[#A32D2D] text-[#A32D2D] text-[11px] uppercase tracking-wide transition-colors duration-300 hover:bg-[#A32D2D]/10 disabled:opacity-40">
                    Cancel Order
                  </button>
                  <button onClick={handleMarkShipped} disabled={updating || !canShip} className="w-full py-4 bg-black text-white text-[11px] uppercase tracking-wide transition-opacity duration-300 hover:opacity-90 disabled:opacity-40">
                    Mark As Shipped
                  </button>
                </div>
              </div>
