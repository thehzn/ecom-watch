              ...
              <thead>
                <tr className="bg-[#EEEEEE] border-b border-[#CFC4C5]">
                  {['Image', 'Product', 'SKU / Brand', 'Model No.', 'Category', 'Price', 'Stock', 'Actions'].map((h) => (
                    <th
                      key={h}
                      /* Price, Stock, Actions are centered so the header
                         label sits on the same visual axis as the
                         centered content below it, regardless of how
                         wide a given price/badge/icon-group is. */
                      className={`px-4 xl:px-6 py-5 text-[11px] font-normal uppercase tracking-[0.1em] text-[#1A1C1C] whitespace-nowrap ${
                        h === 'Price' || h === 'Stock' || h === 'Actions'
                          ? 'text-center'
                          : 'text-left'
                      }`}
                    >
              ...
              {paginatedProducts.map((p) => (
                <div key={p._id} className="p-4 flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {/* SKU & Brand */}
                    <td className="px-4 xl:px-6 py-4 align-middle overflow-hidden">
                      <p className="text-sm font-bold text-black truncate" title={p.sku}>
                        {p.sku}
                      </p>
                    ...
                    {/* Model Number */}
                    <td className="px-4 xl:px-6 py-4 align-middle overflow-hidden">
                      <p className="text-sm text-[#1A1C1C] truncate" title={p.modelNumber}>
                        {p.modelNumber}
                      </p>
                    </td>
                    ...
                    {/* Price — centered to match header */}
                    ...
                    {/* Stock Status — centered to match header */}
        ...
        <div className="md:hidden">
          ...
          {!loading && !error && paginatedProducts.length > 0 && (
            <div className="space-y-4">
              {paginatedProducts.map((p) => (
                <div key={p._id} className="bg-white border border-[#CFC4C5] p-4 sm:p-5">
                  <div className="flex gap-4">
                    ...
                    />
                    <div className="min-w-0 flex-1">
                      <p className="text-base font-medium text-black leading-6 break-words">
                        {p.modelName}
                      </p>
                      <p className="text-[11px] text-[#5E5E5E] mt-1">{p.category}</p>
                      <p className="text-xs text-black mt-3 break-all">SKU: {p.sku}</p>
                      <p className="text-[11px] text-[#5E5E5E] mt-1">{p.brand}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-x-4 gap-y-4 mt-5 pt-4 border-t border-[#E2DADB]">
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.08em] text-[#6B6B6B] mb-1">Model No.</p>
                      <p className="text-xs text-black break-all">{p.modelNumber}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.08em] text-[#6B6B6B] mb-1">Category</p>
                      <p className="text-xs text-black">{p.category}</p>
                      <p className="text-[11px] text-[#5E5E5E] mt-1">{p.productFor}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.08em] text-[#6B6B6B] mb-1">Price</p>
                      <p className="text-lg text-black" style={{ fontFamily: "'Libre Caslon Text', serif" }}>
                        {formatPrice(p.price)}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.08em] text-[#6B6B6B] mb-1">Stock</p>
                      <StockBadge stock={p.stock} />
                    </div>
                  </div>
...
function Pagination({ currentPage, totalPages, setPage, filteredCount }) {
  return (
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 px-4 py-4 sm:px-6 bg-[#EEEEEE] border-t border-[#CFC4C5] mt-4 md:mt-0">
      <p className="text-sm text-[#5E5E5E]">
        {filteredCount === 0
          ? 'Showing 0 products'
