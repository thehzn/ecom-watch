...
// Matches the backend's password complexity rule exactly (12+ chars, uppercase, lowercase, digit, special symbol)
...
export default function AdminForgotPassword() {
  ...
  const emailForm = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      try {
        ...
        setStep('otp');
        toast.success('Security code sent to administrator email');
      } catch (err) {
  ...
  const otpForm = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      try {
        ...
        setStep('reset');
        toast.success('Security OTP verified');
      } catch (err) {
        setSubmitError(err.message || 'Something went wrong. Please try again.');
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast.success('Fresh security code sent');
    } catch (err) {
          ...
          {step === 'email' && (
            <>
              ...
              <form onSubmit={emailForm.handleSubmit} noValidate className="flex flex-col gap-6">
                ...
                {submitError && (
                  <p className="-mt-4 text-[10px] text-red-600 break-words" style={{ fontFamily: 'Inter, sans-serif' }}>
                    {submitError}
                  </p>
                )}
          ...
          {step === 'otp' && (
            <>
              <div className="flex flex-col items-center text-center mb-6">
                ...
                <p
                  className="mt-4 text-[13px] leading-5 text-[#5D5E63] break-words"
                  style={{ fontFamily: 'Inter, sans-serif' }}
                >
              ...
              <form onSubmit={otpForm.handleSubmit} noValidate className="flex flex-col gap-6">
                ...
                {submitError && (
                  <p className="text-center text-[10px] text-red-600 break-words" style={{ fontFamily: 'Inter, sans-serif' }}>
                    {submitError}
                  </p>
                )}
          ...
          {step === 'reset' && (
            <>
              ...
              <form onSubmit={resetForm.handleSubmit} noValidate className="flex flex-col gap-5">
                ...
                {submitError && (
                  <p className="text-[10px] text-red-600 break-words" style={{ fontFamily: 'Inter, sans-serif' }}>
                    {submitError}
                  </p>
                )}
