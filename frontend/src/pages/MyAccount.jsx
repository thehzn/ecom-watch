...
//    Flow: enter new number -> send OTP -> verify OTP -> update
//    ========================================================= */
...
//   const otpInputClass =
//     "w-full bg-[#141720] border border-white/15 focus:border-white text-white text-sm rounded-xl px-4 py-3 outline-none transition-colors placeholder:text-gray-600 text-center tracking-widest";
...
//       return;
//     }
//     if (newMobileNumber === currentMobile) {
//       setError("New mobile number cannot be same as current number");
//       return;
//     }

//     setLoading(true);
//     try {
//       await post("/apiuser/user/send-mobile-change-otp", { newMobileNumber });
//       setStep("otp");
//     } catch (err) {
...
//       // Step 2: actually update the mobile number
//       const res = await put("/apiuser/user/update-mobile-number", {
//         newMobileNumber,
//       });

//       setStep("success");
//       onUpdated(res?.user, newMobileNumber);
//     } catch (err) {
...
//       <div
//         onClick={(e) => e.stopPropagation()}
//         className="w-full max-w-md bg-[#0E1015] border border-white/20 rounded-3xl p-6 sm:p-10 shadow-2xl relative"
//       >
//         <button
//           type="button"
//           onClick={onClose}
...
//           <div>
//             <h3 className="text-xl font-bold text-white tracking-tight">
//               {step === "success" ? "Number Updated" : "Update Mobile Number"}
//             </h3>
//             <p className="text-xs text-gray-400">
//               {step === "input" && "Enter your new mobile number"}
//               {step === "otp" && "OTP sent to your registered email"}
//               {step === "success" && "Your mobile number has been changed"}
//             </p>
//           </div>
//         </div>
...
//             <p className="text-xs text-gray-400">
//               New number: <span className="text-white font-semibold">{newMobileNumber}</span>
//             </p>
...
//   // Persist the new mobile number into the auth slice (and localStorage
//   // via setAuthStorage inside the reducer) so it survives a refresh.
...
//       } catch (err) {
//         const message = err.message || "Could not save address. Please try again.";
...
//     }
//     if (newMobileNumber === currentMobile) {
//       const message = "New mobile number cannot be same as current number";
//       setError(message);
//       toast.error(message);
//       return;
//     }
...
//     } catch {
//       setAddressesError("Unable to load addresses right now.");
//       toast.error("Unable to load addresses right now.");
//     } finally {
//       setAddressesLoading(false);
//     }
//   };
...
//           {/* Membership Standards Sub-Strip */}
//           <div className="mt-8 pt-6 border-t border-white/10 grid grid-cols-1 sm:grid-cols-2 gap-4 justify-content  text-center sm:text-left">
//             {/* <div>
//               <p className="text-[10px] uppercase tracking-widest text-gray-400 font-bold">ACCOUNT TIER</p>
//               <p className="text-sm font-semibold text-white mt-0.5">Geneva Collector</p>
//             </div> */}
...
//         <div className="pt-6 border-t border-white/10 flex items-center justify-between">
//           <p className="text-xs text-gray-500">
//             Signed in as <span className="text-gray-300 font-semibold">{user.email || "Guest"}</span>
//           </p>
//           <button
//             onClick={handleLogout}
//             className="inline-flex items-center gap-2 border border-red-500/30 hover:bg-red-500/10 text-red-400 text-xs font-bold uppercase tracking-wider px-6 py-3 rounded-full transition-all"
...
function AddressFormModal({ initialValues, onClose, onSaved, post, put }) {
  ...
  // Wrap Formik's submit so a failed validation also fires a toast,
  // in addition to the inline field errors Formik already shows.
