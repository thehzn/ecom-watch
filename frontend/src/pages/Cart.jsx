...
export default function Cart() {
  ...
  const handleUpdateQuantity = async (productId, nextQty) => {
    ...
    try {
      await patch("/apicarts/updatequantity", {
        ProductId: productId,
        quantity: nextQty,
      });
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-[70vh] bg-[#08090C] flex flex-col items-center justify-center text-center px-6 ">
        ...
        <h2 className="font-caslon text-3xl sm:text-4xl text-white">
          Your Shopping Bag is Empty
        </h2>

        <p className="mt-3 text-sm text-white/60 max-w-sm font-normal">
          You haven't reserved any timepieces yet. Explore our haute
          horlogerie collections.
        </p>
  ...
  return (
    <div className="min-h-screen bg-[#08090C] text-white py-12 px-5 sm:px-8 ">
      <div className="max-w-7xl mx-auto">
        <h1 className="font-caslon text-3xl sm:text-4xl text-white mb-8">
          ...
          <div className="lg:col-span-8 flex flex-col gap-4">
            {items.map(({ product, quantity }) => {
              ...
              return (
                <div
                  key={product._id}
                  className="bg-[#0E1015] border border-white/10 rounded-xl p-5 sm:p-6 flex flex-col sm:flex-row items-center gap-6 hover:border-white/20 transition-colors"
                >
                  <img
                  ...
                  <div className="flex-1 flex flex-col gap-1 text-center sm:text-left">
                    ...
                    <h3 className="font-caslon text-xl text-white">
                      {product.modelName}
                    </h3>
          ...
          <div className="lg:col-span-4">
            <div className="bg-[#0E1015] border border-white/20 rounded-xl p-6 sm:p-8 flex flex-col gap-6 shadow-2xl border-white/10">
              ...
              <button
                ...
                className="w-full bg-[#C5A880] hover:bg-[#d8bd95] text-black text-xs font-bold uppercase tracking-[0.2em] py-4 rounded-lg shadow-lg shadow-[#C5A880]/10 transition-all"
              >
