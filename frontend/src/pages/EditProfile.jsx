...
export default function EditProfile() {
  ...
  // Fires a toast alongside Formik's inline field errors when the
  // personal-details form is submitted with invalid values.
  ...
  const handleChangeEmail = async () => {
    ...
    if (!emailOtpVerified) {
      const message = 'Please verify the OTP before changing your email.';
      setEmailError(message);
      toast.error(message);
      return;
    }
    ...
    if (!cleanEmail) {
      const message = 'Please enter your new email address.';
      setEmailError(message);
      toast.error(message);
      return;
    }
