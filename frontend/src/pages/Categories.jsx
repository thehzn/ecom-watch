...
function ProductCard({ product }) {
  ...
  return (
    <Link
      ...
      className="group block bg-[#0E1015] border border-white/10 hover:border-[#C5A880]/50 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-2xl flex flex-col justify-between"
    >
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#141720] p-8 flex items-center justify-center">
        ...
        <button
          ...
          className="absolute right-3.5 top-3.5 w-8 h-8 rounded-full bg-black/60 backdrop-blur-md border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-black transition-all"
        >
          <Heart
            size={14}
      ...
      <div className="p-5 flex flex-col gap-1.5">
        ...
        <h3 className="font-caslon text-base text-white/90 group-hover:text-white transition-colors line-clamp-1 mt-1">
          {product.modelName}
        </h3>

        {details && (
          <p className="text-xs text-white/40 line-clamp-1 mt-0.5">{details}</p>
        )}

        <div className="mt-2 pt-2 border-t border-white/10 flex items-center justify-between">
          <p className="font-caslon text-base text-white">
            ₹{Number(product.price).toLocaleString()}
          </p>

          <span className="text-[10px] uppercase tracking-wider text-white/60 font-semibold group-hover:text-[#E4CA99] transition-colors">
            Configure &rarr;
          </span>
        </div>
      </div>
...
export default function Categories() {
  ...
  // This points to the LOWER product listing section
    ...
    async (category) => {
      ...
      try {
        ...
        const data = await get(
          `/apiproduct/getallproducts?${params.toString()}`
        );
  ...
  useEffect(() => {
    if (initialCategory) {
      const match = COLLECTIONS.find(
        (c) =>
          c.category.toLowerCase() === initialCategory.toLowerCase()
      );
  ...
  /*
    ...
    It loads that category and then scrolls
    DOWN to the Product Listing section.
  */
    ...
    <div className="min-h-screen w-full bg-[#08090C] text-white font-['Plus_Jakarta_Sans']">
      ...
      <section className="w-full bg-[#0B0D12] border-b border-white/10 px-6 py-16 sm:py-24 text-center relative overflow-hidden">
        <div className="relative mx-auto flex max-w-3xl flex-col items-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#C5A880]/10 border border-[#C5A880]/20 text-[10px] font-semibold uppercase tracking-[0.25em] text-[#E4CA99] mb-3">
            <Sparkles size={12} />
            Haute Horlogerie Pillars
          </div>

          <h1 className="font-caslon text-4xl sm:text-6xl text-white tracking-tight">
            The Collections
          </h1>

          <p className="mt-4 text-sm sm:text-base text-white/60 font-normal leading-relaxed">
            Explore our four foundational horology collections: Luxury,
            Heritage, Sports, and Contemporary.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 flex flex-col gap-16">
        {COLLECTIONS.map((c, idx) => (
          ...
          >
            ...
            >
              <div className="aspect-square rounded-xl overflow-hidden border border-white/15 shadow-2xl bg-[#141720]">
                <img
                  src={c.image}
                  alt={c.title}
            ...
            >
              ...
              <h2 className="font-caslon mt-2 text-2xl sm:text-4xl text-white tracking-tight">
                {c.title}
              </h2>
      ...
      <section
        ref={listingRef}
        className="w-full border-t border-white/10 bg-[#0B0D12] px-6 py-16"
      >
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4">
            <div>
              ...
              <h2 className="font-caslon text-2xl sm:text-4xl text-white tracking-tight mt-1">
          ...
          {loading && (
            <div className="py-20 text-center">
              ...
              <p className="text-sm text-gray-400">
                Loading {selectedCategory} models…
              </p>
            </div>
          )}
          ...
          {!loading &&
            !fetchError &&
            products.length === 0 && (
              <div className="bg-[#0E1015] border border-white/10 rounded-xl p-12 text-center max-w-lg mx-auto">
                <p className="text-sm text-gray-300">
                  No timepieces currently listed under{' '}
                  {selectedCategory}.
                </p>

                <Link
                  to="/shop"
                  className="mt-4 inline-block text-xs uppercase font-bold text-[#E4CA99] hover:underline tracking-wider"
                >
