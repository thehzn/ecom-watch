...
export const Adminlogin = async ( req, res ) =>{
   try {
      ...
      if(!isMatch) return res.status(400).json({ status:false, message:"Invalid Password"})
         const AdminToken = jwt.sign({ id:currentAdmin._id , role:"admin" },process.env.JWT_SECRET,{expiresIn:"1d"} )
      return res.status(200).json({ status:true, message:"Successfully Loggedin",token:AdminToken,
         user:{
